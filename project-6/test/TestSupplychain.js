...
contract('SupplyChain', function(accounts) {
    ...
    const originFarmerID = accounts[3];
    ...
    const productPrice1 = web3.toWei('1', "ether");
    const productPrice2 = web3.toWei('1.5', "ether");
    const productPrice3 = web3.toWei('2', "ether");
    const productPrice4 = web3.toWei('2.5', "ether");
    const productPrice5 = web3.toWei('3', "ether");
    var itemState = 0;
    ...
    describe('Testing SupplyChain smart contract functions to check the journey of a item through the supply chain flow.', () => {
        ...
        it('Check a poulterer has been added to the smart contract', async () => {
            ...
            tx = await this.contract.addpoulterer(originFarmerID, {from: ownerID});

            assert.equal(tx.logs[0].event, 'poultererAdded');
            assert.equal(tx.logs[0].args.account, originFarmerID);
        });
        ...
        it('Call function storeItem() that allows a poulterer to store metal', async () => {
            tx = await this.contract.processItem(upc, {from: originPoultererID});
        ...
        it('Call function testMilk() that allows a dairy factory to do milk quality test and mark the milk as qualified', async () => {
            // Call function packItem()
            tx = await this.contract.testMilk(upc, {from: dairyfactoryID, value: true});

            assert.equal(tx.logs[0].event, 'Qualified');
            assert.equal(tx.logs[0].args.upc, upc);
            assert.equal(tx.logs[0].args.emiter, dairyfactoryID);
            ...
            // Verify the result set. Item state will be Qualified 
            assert.equal(resultBufferOne[8], 7, 'Error: Invalid item State');
        });

        it('Call function testMilk() that allows a dairy factory to do milk quality test and mark the milk as failed', async () => {
            // Call function packItem()
            tx = await this.contract.testMilk(upc, {from: dairyfactoryID, value: false});

            assert.equal(tx.logs[0].event, 'Failed');
            assert.equal(tx.logs[0].args.upc, upc);
            assert.equal(tx.logs[0].args.emiter, dairyfactoryID);
            ...
            // Verify the result set. Item state will be Failed 
        ...
        it('Call function returnMilk() that allows a dairy factory to return bad quality milk to the poulterer.', async () => {
            // Call function returnMilk()
            tx = await this.contract.returnMilk(upc, {from: dairyfactoryID});

            assert.equal(tx.logs[0].event, 'Returned');
            assert.equal(tx.logs[0].args.upc, upc);
            assert.equal(tx.logs[0].args.emiter, dairyfactoryID);
            ...
            // Verify the result set. Item state will be Returned 
            assert.equal(resultBufferOne[8], 10, 'Error: Invalid item State');
        });

        it('Call function retakeMilk() that allows a poulterer retake bad quality milk from the dairy factory.', async () => {
            // Call function retakeMilk()
            tx = await this.contract.retakeMilk(upc, {from: originPoultererID});

            assert.equal(tx.logs[0].event, 'Retaken');
            assert.equal(tx.logs[0].args.upc, upc);
            assert.equal(tx.logs[0].args.emiter, originPoultererID);
            ...
            // Verify the result set. Item state will be Retaken 
            assert.equal(resultBufferOne[8], 11, 'Error: Invalid item State');
        });

        it('Call function shipItemToPoulterer() that allows a dairy factory to ship bad quality milk to poulterer', async () => {
            // Call function shipItemToPoulterer()
            tx = await this.contract.shipItemToPoulterer(upc, {from: dairyfactoryID});

            assert.equal(tx.logs[0].event, 'ShippedToPoulterer');
            assert.equal(tx.logs[0].args.upc, upc);
            assert.equal(tx.logs[0].args.emiter, dairyfactoryID);
            ...
            // Verify the result set. Item state will be ShippedToPoulterer 
            assert.equal(resultBufferOne[8], 12, 'Error: Invalid item State');
        });

        it('Call function receiveItemByPoulterer() that allows a poulterer to mark item as received.', async () => {
            // Call function receiveItemByPoulterer()
            tx = await this.contract.receiveItemByPoulterer(upc, {from: originPoultererID});

            assert.equal(tx.logs[0].event, 'ReceivedByPoulterer');
            assert.equal(tx.logs[0].args.upc, upc);
            assert.equal(tx.logs[0].args.emiter, originPoultererID);
            ...
            // Verify the result set. Item state will be ReceivedByPoulterer 
            assert.equal(resultBufferOne[8], 13, 'Error: Invalid item State');
        });

        it('Call function destroyMilk() that allows a poulterer to destroy the bad quality milk.', async () => {
            // Call function destroyMilk()
            tx = await this.contract.destroyMilk(upc, {from: originPoultererID});

            assert.equal(tx.logs[0].event, 'Destroyed');
            assert.equal(tx.logs[0].args.upc, upc);
            assert.equal(tx.logs[0].args.emiter, originPoultererID);
            ...
            // Verify the result set. Item state will be Destroyed 
            assert.equal(resultBufferOne[8], 14, 'Error: Invalid item State');
        });
        ...
        it('Call function receiveItemByDistributor() that allows a distributor to mark item as received.', async () => {
            ...
            assert.equal(tx.logs[0].event, 'RecievedByDistributor');
            assert.equal(tx.logs[0].args.upc, upc);
            assert.equal(tx.logs[0].args.emiter, distributorID);
            ...
            // Verify the result set. Item state will be RecievedByDistributor 
        ...
        it('Call function receiveItemByRetailer() that allows a distributor to mark item as received.', async () => {
            ...
            assert.equal(tx.logs[0].event, 'RecievedByRetailer');
            assert.equal(tx.logs[0].args.upc, upc);
            assert.equal(tx.logs[0].args.emiter, retailerID);
            ...
            // Verify the result set. Item state will be RecievedByRetailer 
        ...
        it('Call function receiveItemBySupermarket() that allows a distributor to mark item as received.', async () => {
            ...
            assert.equal(tx.logs[0].event, 'RecievedBySupermarket');
            assert.equal(tx.logs[0].args.upc, upc);
            assert.equal(tx.logs[0].args.emiter, supermarketID);
            ...
            // Verify the result set. Item state will be RecievedBySupermarket 
        ...
        it('Call function fetchItemBufferDistributor() that allows a distributor to fetch data from the supply chain smart contract.', async () => {
            ...
            assert.equal(resultBufferForDistributor[2], originPoultererID, 'Error: Invalid ownerID');
            assert.equal(resultBufferForDistributor[3], originPoultererID, 'Error: Invalid originPoultererID');
